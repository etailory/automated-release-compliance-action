...
- CI workflow now triggers on `release: [published]` instead of every push to
  `master`, matching how the action evaluates release context.
- Workflow and usage examples pin to the floating `@v1` tag so they pick up
...
- Updated repository owner references from `markgrendev` to `etailory` across the
  README, action metadata (`author`), examples, and changelog links.

### Added

- CI workflow ([`.github/workflows/release-compliance.yml`](.github/workflows/release-compliance.yml))
  that runs the compliance action on every push to `master`.
...
- **ESLint + TypeScript linting** via `bun run lint`

[1.0.2]: https://github.com/etailory/automated-release-compliance-action/releases/tag/v1.0.2
[1.0.1]: https://github.com/etailory/automated-release-compliance-action/releases/tag/v1.0.1
[1.0.0]: https://github.com/etailory/automated-release-compliance-action/releases/tag/v1.0.0
